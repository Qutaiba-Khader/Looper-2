    ...
    Public Function FixLooperFile(inputPath As String, replaceOriginal As Boolean) As FixResult
        ...
        For lineIdx = 0 To lines.Length - 1
            ...
            If parts Is Nothing OrElse parts.Length < 4 Then
                Log("  Line " & lineIdx & ": skipped (< 4 parts)")
                fixedLines.Add(line)
                Continue For
            End If
            ...
            Log("  Line " & lineIdx & ": BROKEN - " & filename)
            ...
            If pathCache.ContainsKey(filename) Then
                ...
                If Not String.Equals(cachedPath, filePath, StringComparison.OrdinalIgnoreCase) Then
                    ...
                    anyChanged = True
                    Log("    CACHED -> " & cachedPath)
                Else
                    ' Cache points to same broken path, still broken
                    fixedLines.Add(line)
                    result.UnfixableEvents += 1
                    Log("    CACHED but same path, still broken")
                End If
                Continue For
            End If
            ...
            Else
                ...
                If bestMatch IsNot Nothing Then
                    Log("    Only found at same broken path")
                Else
                    Log("    NO EXACT MATCH in results")
                End If
            End If
        Next
